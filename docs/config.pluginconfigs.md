...
|  [importHelpers?](./config.pluginconfigs.importhelpers.md) |  | 'inline' \| 'auto' \| 'cdn' \| 'node' \| string | _(Optional)_ Import emit helpers (e.g. <code>\_import</code>, <code>\__dynamicImportTransform</code>, etc..) from runtime (a local file in this package). |
