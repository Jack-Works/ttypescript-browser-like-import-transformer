...
function __dynImportTransform(path) {
    ...
    const result // browser style import
     = runtimeTransform(config, path, dynamicImport);
    if (result === null)
        return dynamicImport //example.com'
        (path);
    return result;
    function parseJS(...a) {
        return null;
    }
    function runtimeTransform(config, path // Node style export
    , dynamicImport) {
